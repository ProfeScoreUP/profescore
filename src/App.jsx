...
export default function App() {
  ...
  async function submitReview() {
    ...
    setSubmitting(true);
    if (editingReview) {
      await supabase.from("resenas").update({ materia: revMateria, rating: selectedStar, texto: revText.trim(), tags: selectedTags, modalidad: revModalidad }).eq("id", editingReview.id);
    } else {
  ...
  return (
    <div className="layout">
              ...
              {currentMateria ? (
                ...
                <div className="materias-grid">
                  ...
                  {getFilteredMaterias().map((m) => {
                    ...
                    return (
                      <div key={m.id} className="materia-card" onClick={() => {setCurrentMateria(m.nombre);setProfTab("todos");}}>
                        <div className="materia-nombre">{m.nombre}</div>
                        <div className="materia-meta">{profsDeMateria.length} profesor{profsDeMateria.length!==1?"es":""}</div>
                      </div>
                    );
                  })}
                </div>
        ...
        <div className="modal-overlay" onClick={() => setShowReviewModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <div className="star-picker">{[1,2,3,4,5].map((n) => <button key={n} className={`star-btn${selectedStar>=n?" active":""}`} onClick={() => setSelectedStar(n)}>★</button>)}</div>
            </div>
            <div className="form-group">
              <label className="form-label">Tags</label>
              <div className="tag-picker">{ALL_TAGS.map((t) => <span key={t} className={`tag-option${selectedTags.includes(t)?" selected":""}`} onClick={() => setSelectedTags(selectedTags.includes(t)?selectedTags.filter((x)=>x!==t):[...selectedTags,t])}>{t}</span>)}</div>
            </div>
      ...
      {/* ADD MATERIA MODAL */}
      {showAddMateriaModal && (
        <div className="modal-overlay" onClick={() => setShowAddMateriaModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-title">Agregar materia</div>
            <div className="form-group">
              <label className="form-label">Nombre de la materia</label>
              <input value={nuevaMateriaDirecta} onChange={(e) => setNuevaMateriaDirecta(e.target.value)} placeholder="Ej: Derecho Laboral" />
            </div>
            <div className="modal-actions">
