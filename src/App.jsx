...
export default function App() {
  ...
  async function fetchMensajes() {
    ...
    const { data } = await supabase.from("mensajes").select("*")
      .or(`de_user_id.eq.${session.user.id},para_user_id.eq.${session.user.id}`)
      .order("created_at", { ascending: true });
    setMensajes(data||[]);
    const unread = (data||[]).filter((m) => m.para_user_id===session.user.id && !m.leido).length;
  ...
  function openUserProfile(userId) {
    ...
    if (p) { setViewingUser(p); setView("perfil-usuario"); setCurrentProf(null); }
  }

  function navigate(v) { setView(v); setCurrentProf(null); setCurrentMateria(null); setSearch(""); setChatWith(null); }
  ...
  function getFilteredProfs() {
    ...
    const withAvg = list.map((p) => { let revs=resenas[p.id]||[]; if(modalidadFilter) revs=revs.filter((r)=>r.modalidad===modalidadFilter); return {...p,avg:avgRating(revs),cnt:revs.length}; });
    if (profTab==="mejor") withAvg.sort((a,b)=>b.avg-a.avg); else if (profTab==="recientes") withAvg.sort((a,b)=>b.cnt-a.cnt);
    return withAvg;
  }
        ...
        {showComs&&<div className="comments-section">
          ...
          {revComs.map((c)=>{const cp=perfilesMap[c.user_id]; return (
            <div key={c.id} className="comment-item">
              <div className="comment-avatar" onClick={()=>c.user_id&&openUserProfile(c.user_id)}>
    ...
    <div className="layout">
      ...
      <div className="sidebar">
        ...
        <div className="sidebar-section">
          ...
          {[["materias","📚","Materias"],["profesores","👨‍🏫","Profesores"],["comunidad","👥","Comunidad"],["acerca","ℹ️","Acerca de"]].map(([v,icon,label])=>(
          ...
          {currentProf ? (
            <>
              ...
              {(()=>{
                ...
                const c=colorFor(idx); const avg=avgRating(filteredProfRevs); const summary=aiSummary(currentProf,filteredProfRevs);
                return (<>
                  ...
                  <div className="stats-row">{[[avg?avg.toFixed(1):"—","calificación",avg?ratingColor(avg):undefined],[filteredProfRevs.length,"reseñas",undefined],[(currentProf.materias||[]).length,"materias",undefined]].map(([v,l,col],i)=><div key={i} className="stat-card"><div className="stat-val" style={col?{color:col}:{}}>{v}</div><div className="stat-lbl">{l}</div></div>)}</div>
            ...
            <div className="about-page">
              <div className="about-hero">
                ...
                <div>
                  <div className="about-title">ProfeScore</div>
                  <div className="about-tagline">La voz de los estudiantes de la Universidad de Palermo</div>
                </div>
              </div>
              <div className="about-section">
                <h3>¿Qué es ProfeScore?</h3>
                <p>ProfeScore es una plataforma creada por y para estudiantes de la Universidad de Palermo. Nuestro objetivo es ayudarte a tomar mejores decisiones al inscribirte en materias, compartiendo experiencias reales sobre cada profesor de forma honesta y respetuosa.</p>
              </div>
              <div className="about-section">
                <h3>Nuestra misión</h3>
                <p>Creemos que la información compartida entre estudiantes tiene un valor enorme. ProfeScore busca centralizar esa información de manera organizada, accesible y confiable, para que cada alumno pueda elegir sus materias con mayor contexto y confianza.</p>
              </div>
              <div className="about-section">
                <h3>Valores</h3>
                <p>Fomentamos la honestidad y la objetividad. Una buena reseña no solo critica o elogia — describe experiencias reales que ayudan a otros. Pedimos a todos los usuarios que sean respetuosos y constructivos en sus opiniones.</p>
              </div>
            ...
            <>
              <div style={{marginBottom:"1.25rem"}}>
                <div className="logo" style={{marginBottom:4}}><div className="dot"/>Mensajes</div>
              </div>
              ...
              : chatWith ? (
                <div className="chat-view">
                  <div className="chat-header">
                    ...
                    <div>
                      <div style={{fontWeight:500,fontSize:14}}>@{chatWith.username}</div>
                      {chatWith.carrera&&<div style={{fontSize:12,color:"#888"}}>{chatWith.carrera}</div>}
                    </div>
                  </div>
                  <div className="chat-messages">
                    {chatMessages.length===0&&<div className="empty" style={{padding:"1rem"}}>Empezá la conversación</div>}
                    {chatMessages.map((m)=>{
                      const mine = m.de_user_id===session.user.id;
                      return (
                ...
                <>
                  {conversations.length===0&&<div className="empty">No tenés conversaciones todavía. Podés iniciar una desde el perfil de otro usuario.</div>}
                  <div className="conversations-list">
                    {conversations.map((conv)=>{
                      const otherPerfil = perfilesMap[conv.otherId];
                      if (!otherPerfil) return null;
                      const mine = conv.lastMsg.de_user_id===session.user.id;
                      return (
                        <div key={conv.otherId} className={`conversation-item${conv.unread>0?" unread":""}`} onClick={()=>openChat(otherPerfil)}>
                          <Avatar url={otherPerfil.foto_url} name={otherPerfil.username} size={40} fontSize={14}/>
                          <div className="conv-info">
                            <div className="conv-username">@{otherPerfil.username}</div>
                            <div className="conv-preview">{mine?"Vos: ":""}{conv.lastMsg.texto}</div>
                          </div>
            ...
            {myComments.length===0?<div className="empty">Todavía no comentaste ninguna reseña</div>:<div style={{display:"flex",flexDirection:"column",gap:8}}>{myComments.map((c)=>{
              const r=allRevenas.find((x)=>x.id===c.resena_id); const prof=r?profesores.find((p)=>p.id===r.profesor_id):null;
              return <div key={c.id} className="review-card">
            ...
            <>
              ...
              <div className="tabs">{[["todos","Todos"],["mejor","Mejor calificados"],["recientes","Más reseñas"]].map(([k,l])=><button key={k} className={`tab${profTab===k?" active":""}`} onClick={()=>setProfTab(k)}>{l}</button>)}</div>
              {loading?<div className="empty">Cargando...</div>:<div className="prof-list">
                ...
                {getFilteredProfs().map((p,i)=>{
                  const c=colorFor(i); let revs=resenas[p.id]||[]; if(modalidadFilter) revs=revs.filter((r)=>r.modalidad===modalidadFilter);
                  ...
                  return <div key={p.id} className="prof-card" onClick={()=>{setCurrentProf(p);setDetailModalidad("");}}>
              ...
              {currentMateria ? (
                <>
                  ...
                  <div className="prof-list">
                    ...
                    {getFilteredProfs().map((p,i)=>{const c=colorFor(i);const revs=resenas[p.id]||[];return<div key={p.id} className="prof-card" onClick={()=>{setCurrentProf(p);setDetailModalidad("");}}>
                      <div className="prof-row">
                        <div className="avatar" style={{background:c.bg,color:c.color}}>{p.foto_url?<img src={p.foto_url} alt={p.nombre}/>:initials(p.nombre)}</div>
                ...
                <div className="materias-grid">
                  {materias.filter((m)=>m.nombre.toLowerCase().includes(search.toLowerCase())).length===0&&<div className="empty">No se encontraron materias</div>}
                  {materias.filter((m)=>m.nombre.toLowerCase().includes(search.toLowerCase())).map((m)=>{
                    const cnt=profesores.filter((p)=>(p.materias||[]).includes(m.nombre)).length;
                    return<div key={m.id} className="materia-card" onClick={()=>{setCurrentMateria(m.nombre);setProfTab("todos");}}>
                      <div className="materia-nombre">{m.nombre}</div>
                      <div className="materia-meta">{cnt} profesor{cnt!==1?"es":""}</div>
                    </div>;
                  })}
                </div>
              )}
      ...
      {showEditProfileModal&&<div className="modal-overlay" onClick={()=>setShowEditProfileModal(false)}><div className="modal" onClick={(e)=>e.stopPropagation()}>
        ...
        <div className="form-group"><label className="form-label">URL de foto de perfil</label><input value={editFotoUrl} onChange={(e)=>setEditFotoUrl(e.target.value)} placeholder="https://i.imgur.com/tu-foto.jpg"/>
      ...
      {showEditProfFotoModal&&<div className="modal-overlay" onClick={()=>setShowEditProfFotoModal(false)}><div className="modal" onClick={(e)=>e.stopPropagation()}>
        ...
        <div className="form-group"><label className="form-label">URL de la foto</label><input value={profFotoUrl} onChange={(e)=>setProfFotoUrl(e.target.value)} placeholder="https://i.imgur.com/foto.jpg"/>
      ...
      {showAuthModal&&<div className="modal-overlay" onClick={()=>setShowAuthModal(false)}><div className="modal" onClick={(e)=>e.stopPropagation()}>
        ...
        {authMode==="register"&&<><div className="info-box">Si usás tu email <strong>@up.edu.ar</strong>, tus reseñas tendrán el badge <span className="badge-up">✓ Alumno UP</span></div><div className="info-box warning">⚠️ No uses una contraseña que tengas en otras cuentas importantes. Esta plataforma es estudiantil — usá una contraseña única.</div></>}
      ...
      {showReviewModal&&<div className="modal-overlay" onClick={()=>setShowReviewModal(false)}><div className="modal" onClick={(e)=>e.stopPropagation()}>
        ...
        {!session&&!editingReview&&<div className="info-box"><strong>Reseña como invitado.</strong> <button className="link-btn" onClick={()=>{setShowReviewModal(false);setShowAuthModal(true);setAuthMode("login");}}>Iniciá sesión</button> para verificarte como Alumno UP.</div>}
        ...
        <div className="form-row">
          <div className="form-group" style={{flex:1}}><label className="form-label">Materia cursada</label><select value={revMateria} onChange={(e)=>setRevMateria(e.target.value)}>{(currentProf?.materias||[]).map((m)=><option key={m}>{m}</option>)}</select></div>
      ...
      {showAddMateriaModal&&<div className="modal-overlay" onClick={()=>setShowAddMateriaModal(false)}><div className="modal" onClick={(e)=>e.stopPropagation()}>
        ...
        <div className="form-group"><label className="form-label">Nombre de la materia</label><input value={nuevaMateriaDirecta} onChange={(e)=>setNuevaMateriaDirecta(e.target.value)} placeholder="Ej: Derecho Laboral"/></div>
      ...
      {showAddProfModal&&<div className="modal-overlay" onClick={()=>setShowAddProfModal(false)}><div className="modal" onClick={(e)=>e.stopPropagation()}>
        ...
        <div className="form-group"><label className="form-label">Área / Departamento</label><input value={newDept} onChange={(e)=>setNewDept(e.target.value)} placeholder="Ej: Contabilidad, Marketing..."/></div>
        <div className="form-group"><label className="form-label">Materias que dicta</label>
          ...
          <div style={{display:"flex",gap:6,marginTop:8}}><input value={nuevaMateria} onChange={(e)=>setNuevaMateria(e.target.value)} placeholder="Nombre de la materia..." style={{flex:1}}/><button className="btn-primary" style={{flex:"none",padding:"6px 12px"}} onClick={addNuevaMateria}>Agregar</button><button className="btn-cancel" onClick={()=>setShowNewMateriaField(false)}>✕</button></div>}
